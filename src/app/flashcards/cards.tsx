...
const SEED_FOLDERS: Folder[] = [
    { id: "f1", name: "Sciences", emoji: "🔬", color: "#34d399", createdAt: Date.now() - 86400000 * 10, expanded: true },
    { id: "f2", name: "Mathematics", emoji: "📐", color: "#60a5fa", createdAt: Date.now() - 86400000 * 7, expanded: false },
];
...
const THEMES: Record<CardTheme, { label: string; gradient: string; accent: string; star: string }> = {
    nebula: { label: "Nebula", gradient: "linear-gradient(135deg,#0d0829,#1a0a3d 40%,#2d1b69 70%,#1a0a3d)", accent: "#a78bfa", star: "#e0d7ff" },
...
export default function LumIUFlashcards() {
    ...
    const [newFolderColor, setNewFolderColor] = useState("#7c5cfc");
    // context menus
    ...
    const T = {
        bg: isDark ? "#05050f" : "#f0f0fa", surface: isDark ? "#0d0d20" : "#ffffff",
        s2: isDark ? "#141428" : "#f5f5ff", s3: isDark ? "#1a1a35" : "#ebebff",
        border: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)",
        b2: isDark ? "rgba(255,255,255,0.14)" : "rgba(0,0,0,0.14)",
        text: isDark ? "#e8e8ff" : "#0a0a1e", muted: isDark ? "#6060a0" : "#7070a0",
        accent: "#7c5cfc", glow: "rgba(124,92,252,0.18)",
        green: isDark ? "#34d399" : "#059669", amber: isDark ? "#fbbf24" : "#d97706",
        red: isDark ? "#f87171" : "#ef4444", blue: isDark ? "#60a5fa" : "#3b82f6",
    };
...
.lfc{font-family:'Outfit',sans-serif;background:${T.bg};color:${T.text};min-height:100vh;overflow-x:hidden;transition:background 0.3s,color 0.3s;}
/* Top */
.ftop{position:sticky;top:0;z-index:100;height:52px;background:${T.surface};border-bottom:1px solid ${T.border};display:flex;align-items:center;padding:0 20px;gap:12px;}
.flogo{font-weight:700;font-size:18px;background:linear-gradient(135deg,#a78bfa,#60a5fa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:-0.5px;}
.flogo-dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:#7c5cfc;box-shadow:0 0 8px #7c5cfc;margin-left:4px;animation:blink 2s infinite;}
@keyframes blink{0%,100%{opacity:1;}50%{opacity:0.4;}}
.fnav{height:28px;padding:0 14px;border-radius:14px;border:none;font-family:'Outfit';font-size:13px;font-weight:500;cursor:pointer;transition:all 0.15s;background:transparent;color:${T.muted};}
.fnav:hover{background:${T.s2};color:${T.text};}.fnav.act{background:${T.glow};color:#a78bfa;}
.fright{margin-left:auto;display:flex;align-items:center;gap:8px;}
.tpill{display:flex;align-items:center;gap:6px;border:1px solid ${T.border};border-radius:20px;padding:4px 12px;cursor:pointer;background:transparent;color:${T.muted};font-size:12px;font-family:'Outfit';transition:all 0.2s;}
.tpill:hover{border-color:#7c5cfc;color:${T.text};}
/* Library */
.libwrap{padding:26px 22px;max-width:1200px;margin:0 auto;}
.shead{display:flex;align-items:center;gap:10px;margin-bottom:14px;}
.stitle{font-size:18px;font-weight:600;}
.badge{padding:2px 10px;border-radius:12px;font-size:11px;font-weight:600;background:${T.glow};color:#a78bfa;}
/* Folder */
.frow{margin-bottom:18px;}
.fhdr{display:flex;align-items:center;gap:8px;padding:9px 13px;border-radius:12px;cursor:pointer;border:1px solid ${T.border};background:${T.surface};margin-bottom:8px;transition:all 0.15s;user-select:none;}
.fhdr:hover{border-color:${T.b2};background:${T.s2};}
.fchev{font-size:9px;color:${T.muted};transition:transform 0.2s;}.fchev.op{transform:rotate(90deg);}
.fdot{width:8px;height:8px;border-radius:50%;flex-shrink:0;}
.fname{font-size:14px;font-weight:600;flex:1;}
.fcnt{font-size:11px;color:${T.muted};}
.fdecks{padding-left:14px;}
/* Deck grid */
.dgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:12px;margin-bottom:6px;}
.dcard{position:relative;overflow:hidden;border-radius:16px;border:1px solid ${T.border};cursor:pointer;transition:transform 0.2s,box-shadow 0.2s;}
.dcard:hover{transform:translateY(-3px);box-shadow:0 14px 44px rgba(0,0,0,0.4);}
.dcinner{position:relative;overflow:hidden;border-radius:16px;height:164px;display:flex;flex-direction:column;justify-content:space-between;padding:15px;}
.demoji{font-size:26px;line-height:1;margin-bottom:4px;}
.dname{font-size:15px;font-weight:600;color:white;text-shadow:0 2px 8px rgba(0,0,0,0.5);}
.ddesc{font-size:11px;color:rgba(255,255,255,0.52);margin-top:3px;}
.dmeta{display:flex;align-items:center;gap:6px;margin-top:9px;}
.dstat{font-size:11px;color:rgba(255,255,255,0.65);}
.dstudybtn{margin-left:auto;padding:5px 11px;border-radius:20px;border:none;font-family:'Outfit';font-size:11px;font-weight:600;cursor:pointer;background:rgba(255,255,255,0.14);color:white;transition:all 0.15s;}
.dstudybtn:hover{background:rgba(255,255,255,0.27);}
.d3dot{position:absolute;top:9px;right:9px;width:26px;height:26px;border-radius:13px;border:none;background:rgba(0,0,0,0.45);color:rgba(255,255,255,0.75);font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity 0.15s;letter-spacing:1.5px;}
.dcard:hover .d3dot{opacity:1;}
.newdbtn{border-radius:16px;border:2px dashed ${T.border};background:transparent;color:${T.muted};cursor:pointer;height:164px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;font-family:'Outfit';transition:all 0.2s;width:100%;}
.newdbtn:hover{border-color:#7c5cfc;color:#a78bfa;background:${T.glow};}
/* Context menu */
.ctxm{position:fixed;z-index:600;background:${T.surface};border:1px solid ${T.b2};border-radius:13px;padding:4px;min-width:168px;box-shadow:0 12px 40px rgba(0,0,0,0.55);}
.ctxi{display:flex;align-items:center;gap:9px;padding:9px 12px;border-radius:8px;font-size:13px;cursor:pointer;color:${T.text};border:none;background:transparent;width:100%;text-align:left;font-family:'Outfit';transition:background 0.1s;}
.ctxi:hover{background:${T.s2};}.ctxi.dan{color:${T.red};}.ctxi.dan:hover{background:rgba(248,113,113,0.1);}
.ctxsep{height:1px;background:${T.border};margin:3px 4px;}
.ctxlabel{padding:4px 12px 2px;font-size:10px;font-weight:600;color:${T.muted};text-transform:uppercase;letter-spacing:0.06em;}
/* Carousel */
...
.citem{position:absolute;width:265px;height:300px;border-radius:20px;overflow:hidden;cursor:pointer;transform-style:preserve-3d;transition:all 0.45s cubic-bezier(0.4,0,0.2,1);}
.ciface{position:absolute;inset:0;border-radius:20px;padding:18px;display:flex;flex-direction:column;backface-visibility:hidden;-webkit-backface-visibility:hidden;}
.cinum{font-size:10px;font-family:'JetBrains Mono';letter-spacing:0.08em;opacity:0.45;margin-bottom:6px;}
.cisub{font-size:11px;opacity:0.65;margin-bottom:8px;}
.cititle{font-size:14px;font-weight:600;line-height:1.4;flex:1;}
.cifooter{display:flex;align-items:center;justify-content:space-between;margin-top:8px;}
.cistatus{font-size:10px;font-weight:600;padding:2px 8px;border-radius:8px;text-transform:uppercase;}
.citag{font-size:10px;padding:2px 6px;border-radius:6px;background:rgba(255,255,255,0.1);color:rgba(255,255,255,0.65);}
.cnav{display:flex;align-items:center;gap:18px;margin-top:18px;justify-content:center;}
.carrow{width:38px;height:38px;border-radius:50%;border:1px solid ${T.border};background:${T.surface};color:${T.text};font-size:15px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s;}
.carrow:hover{border-color:#7c5cfc;background:${T.glow};color:#a78bfa;}
...
/* Back btn */
.bbtn{width:34px;height:34px;border-radius:50%;border:1px solid ${T.border};background:${T.surface};color:${T.text};font-size:15px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s;flex-shrink:0;}
.bbtn:hover{border-color:#7c5cfc;color:#a78bfa;}
/* Study */
.swrap{min-height:calc(100vh - 52px);display:flex;flex-direction:column;align-items:center;padding:26px 22px;}
.pbbg{flex:1;height:4px;border-radius:2px;background:${T.border};}
.pbfill{height:4px;border-radius:2px;background:linear-gradient(90deg,#7c5cfc,#60a5fa);transition:width 0.4s;}
...
.scface{position:absolute;inset:0;border-radius:20px;padding:26px;backface-visibility:hidden;-webkit-backface-visibility:hidden;display:flex;flex-direction:column;justify-content:center;}
.scback{position:absolute;inset:0;border-radius:20px;padding:26px;backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:rotateY(180deg);display:flex;flex-direction:column;justify-content:center;}
.sq{font-size:20px;font-weight:600;line-height:1.4;text-align:center;color:white;text-shadow:0 2px 10px rgba(0,0,0,0.4);}
.sa{font-size:15px;line-height:1.65;color:white;text-shadow:0 1px 6px rgba(0,0,0,0.3);}
.rrow{display:flex;gap:9px;justify-content:center;flex-wrap:wrap;max-width:520px;width:100%;}
.rbtn{flex:1;min-width:88px;padding:11px 12px;border-radius:13px;border:none;cursor:pointer;font-family:'Outfit';font-size:13px;font-weight:600;transition:all 0.15s;display:flex;flex-direction:column;align-items:center;gap:2px;}
.rbtn span{font-size:10px;opacity:0.65;font-weight:400;}.rbtn:hover{transform:translateY(-2px);}
.hbtn{padding:7px 15px;border-radius:20px;border:1px solid ${T.border};background:transparent;color:${T.muted};font-family:'Outfit';font-size:12px;cursor:pointer;margin-bottom:13px;transition:all 0.15s;}
.hbtn:hover{border-color:#7c5cfc;color:#a78bfa;}
/* Done */
.dwrap{display:flex;flex-direction:column;align-items:center;padding:52px 22px;gap:16px;}
.dstats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;width:100%;max-width:360px;}
.dstat{background:${T.surface};border:1px solid ${T.border};border-radius:12px;padding:11px;text-align:center;}
.dsn{font-size:22px;font-weight:700;}.dsl{font-size:11px;color:${T.muted};margin-top:2px;}
/* Form */
.fwrap{padding:22px;max-width:640px;margin:0 auto;}
.fg{margin-bottom:15px;}
.fl{display:block;font-size:11px;font-weight:600;color:${T.muted};margin-bottom:5px;text-transform:uppercase;letter-spacing:0.06em;}
.fi{width:100%;padding:10px 12px;background:${T.surface};border:1px solid ${T.border};border-radius:10px;color:${T.text};font-family:'Outfit';font-size:14px;outline:none;transition:border-color 0.15s;resize:vertical;}
.fi:focus{border-color:#7c5cfc;}
.tgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;}
.topt{height:50px;border-radius:10px;cursor:pointer;border:2px solid transparent;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;font-size:10px;font-weight:600;color:white;transition:all 0.15s;font-family:'Outfit';}
.topt.sel{border-color:white;box-shadow:0 0 0 3px rgba(255,255,255,0.2);}
.factions{display:flex;gap:10px;margin-top:20px;}
.btnp{padding:10px 22px;border-radius:10px;border:none;background:#7c5cfc;color:white;font-family:'Outfit';font-size:14px;font-weight:600;cursor:pointer;transition:all 0.15s;}
.btnp:hover{background:#6d4ef0;transform:translateY(-1px);}.btnp:disabled{opacity:0.5;cursor:not-allowed;transform:none;}
.btns{padding:10px 16px;border-radius:10px;border:1px solid ${T.border};background:transparent;color:${T.text};font-family:'Outfit';font-size:14px;cursor:pointer;transition:all 0.15s;}
.btns:hover{background:${T.s2};}
/* Create method sheet */
.moverlay{position:fixed;inset:0;z-index:300;background:rgba(0,0,0,0.76);backdrop-filter:blur(6px);display:flex;align-items:flex-end;justify-content:center;}
.msheet{background:${T.surface};border-radius:24px 24px 0 0;padding:20px 18px 34px;width:100%;max-width:500px;border-top:1px solid ${T.b2};}
.mhandle{width:34px;height:4px;border-radius:2px;background:${T.b2};margin:0 auto 18px;}
.mtitle{font-size:17px;font-weight:700;margin-bottom:5px;text-align:center;}
.msub{font-size:12px;color:${T.muted};text-align:center;margin-bottom:20px;}
.mgrid{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-bottom:9px;}
.mbtn{border-radius:15px;border:1px solid ${T.border};background:${T.s2};cursor:pointer;padding:18px 12px;display:flex;flex-direction:column;align-items:center;gap:9px;transition:all 0.2s;font-family:'Outfit';}
.mbtn:hover{border-color:#7c5cfc;background:${T.glow};transform:translateY(-2px);}
.micon{width:46px;height:46px;border-radius:13px;display:flex;align-items:center;justify-content:center;font-size:21px;}
.mlabel{font-size:13px;font-weight:600;color:${T.text};}
.msubtext{font-size:11px;color:${T.muted};text-align:center;}
.mfull{width:100%;border-radius:15px;border:1px solid ${T.border};background:${T.s2};cursor:pointer;padding:16px 18px;display:flex;align-items:center;gap:12px;transition:all 0.2s;font-family:'Outfit';}
.mfull:hover{border-color:#7c5cfc;background:${T.glow};}
.mficon{width:42px;height:42px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:19px;flex-shrink:0;}
/* Card list */
.clist{display:flex;flex-direction:column;gap:5px;margin-top:12px;}
.crow{background:${T.surface};border:1px solid ${T.border};border-radius:11px;padding:10px 13px;display:flex;align-items:center;gap:9px;cursor:grab;transition:background 0.1s;}
.crow:active{cursor:grabbing;}.crow.dov{border-color:#7c5cfc;background:${T.glow};}
...
.crstatus{font-size:10px;font-weight:600;padding:2px 7px;border-radius:6px;text-transform:uppercase;white-space:nowrap;}
.dhandle{color:${T.muted};font-size:13px;cursor:grab;padding:0 3px;flex-shrink:0;}
.c3dot{width:28px;height:28px;border-radius:8px;border:1px solid ${T.border};background:transparent;color:${T.muted};font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s;letter-spacing:1.5px;flex-shrink:0;}
.c3dot:hover{background:${T.s2};color:${T.text};}
/* AI */
.aiwrap{padding:22px;max-width:640px;margin:0 auto;}
.aipulse{width:10px;height:10px;border-radius:50%;background:#a78bfa;box-shadow:0 0 12px #a78bfa;animation:blink 1.5s infinite;}
.aigenbtn{width:100%;padding:13px;border-radius:12px;border:none;background:linear-gradient(135deg,#7c5cfc,#60a5fa);color:white;font-family:'Outfit';font-size:15px;font-weight:600;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;}
.aigenbtn:hover{opacity:0.9;transform:translateY(-1px);}.aigenbtn:disabled{opacity:0.5;cursor:not-allowed;transform:none;}
@keyframes spin{to{transform:rotate(360deg);}}
/* Modal */
.moverlay2{position:fixed;inset:0;z-index:400;background:rgba(0,0,0,0.72);backdrop-filter:blur(5px);display:flex;align-items:center;justify-content:center;padding:14px;}
.mbox{background:${T.surface};border:1px solid ${T.b2};border-radius:20px;padding:22px;width:100%;max-width:460px;max-height:90vh;overflow-y:auto;}
.mboxtitle{font-size:16px;font-weight:700;margin-bottom:16px;}
...
`;
        ...
        return (
            <div>
                ...
                <div className="cnav">
                    ...
                    <div className="cdots">{sorted.map((_, i) => <button key={i} className="cdot" style={{ width: i === carouselIndex ? 18 : 6, background: i === carouselIndex ? "#7c5cfc" : T.border }} onClick={() => setCarouselIndex(i)} />)}</div>
    ...
    const renderAI = () => (
        <div className="aiwrap">
            ...
            <div className="fg"><label className="fl">Emoji</label><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{["✨", "🔬", "📐", "🌌", "⚗️", "🧠", "📚", "💡", "🎯", "🌿", "⚡", "🔮"].map(e => <button key={e} onClick={() => setAiEmoji(e)} style={{ width: 33, height: 33, border: `1px solid ${aiEmoji === e ? "#7c5cfc" : T.border}`, borderRadius: 8, background: aiEmoji === e ? T.glow : "transparent", fontSize: 16, cursor: "pointer", transition: "all 0.15s" }}>{e}</button>)}</div></div>
                ...
                {methodStep === "pick" && <>
                    ...
                    <div className="mgrid">
                        <button className="mbtn" onClick={() => { setView("create"); setShowCreateMethod(false); setMethodStep("pick"); }}>
                            <div className="micon" style={{ background: "linear-gradient(135deg,#7c5cfc,#a78bfa)" }}>⌨️</div>
            ...
            <div className="lfc" onClick={closeMenus}>
                <div className="ftop">
                    ...
                    <div className="fright">
                        ...
                        <button className="tpill" onClick={() => setAppTheme(appTheme === "dark" ? "light" : "dark")}>{appTheme === "dark" ? "🌙 Dark" : "☀️ Light"}</button>
                    </div>
                </div>
                ...
                {showNewDeck && (
                    <div className="moverlay2" onClick={() => setShowNewDeck(false)}>
                        <div className="mbox" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="fg"><label className="fl">Emoji</label><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{["📚", "🔬", "📐", "⚗️", "🧠", "💡", "🎯", "🌿", "⚡", "🔮", "🌌", "✨"].map(e => <button key={e} onClick={() => setNewDeckEmoji(e)} style={{ width: 33, height: 33, border: `1px solid ${newDeckEmoji === e ? "#7c5cfc" : T.border}`, borderRadius: 8, background: newDeckEmoji === e ? T.glow : "transparent", fontSize: 16, cursor: "pointer" }}>{e}</button>)}</div></div>
                ...
                {showNewFolder && (
                    <div className="moverlay2" onClick={() => setShowNewFolder(false)}>
                        <div className="mbox" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="fg"><label className="fl">Emoji</label><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{["📁", "🗂️", "📂", "🔬", "📐", "🌌", "⚗️", "📚", "🎯", "🧬", "💊", "🌿"].map(e => <button key={e} onClick={() => setNewFolderEmoji(e)} style={{ width: 33, height: 33, border: `1px solid ${newFolderEmoji === e ? "#7c5cfc" : T.border}`, borderRadius: 8, background: newFolderEmoji === e ? T.glow : "transparent", fontSize: 16, cursor: "pointer" }}>{e}</button>)}</div></div>
                            <div className="fg"><label className="fl">Colour</label><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{["#7c5cfc", "#34d399", "#60a5fa", "#f87171", "#fbbf24", "#e040fb", "#22d3ee", "#fb923c"].map(c => <button key={c} onClick={() => setNewFolderColor(c)} style={{ width: 28, height: 28, borderRadius: "50%", background: c, border: `2px solid ${newFolderColor === c ? "white" : "transparent"}`, cursor: "pointer", boxShadow: newFolderColor === c ? "0 0 0 2px " + c : "none" }} />)}</div></div>
